...
// 6 Computed 可以缓存，只是把数据挂载了 vm 上
function Computed () {
  let compute = this.$options.computed // 是个对象
  let keys = Object.keys(compute) // ["hello"]
  console.log(compute.hello)

  keys.forEach(key => {
    // 判断是函数，还是对象, this: vm
    let type = typeof compute[key] === 'function'

    Object.defineProperty(this, key, {
      // 是对象要调用 get 方法
      get: type ? compute[key] : compute[key].get,
