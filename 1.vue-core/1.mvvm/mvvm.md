...
* 特点： 数据影响视图，视图影响数据：双向数据绑定



## # MVVM实现思路

1. ` Object.defineProperty() ` 劫持各个属性的 setter / getter
  - IE 8 不支持此 API，且无法通过 polyfill 实现
  - [深入响应式原理](https://cn.vuejs.org/v2/guide/reactivity.html)

2. 数据劫持 ` Observe `
3. 数据代理，在数据变动时发布消息给订阅者，触发相应的监听回调
4. 编译模板
5. 发布订阅模式
6. 链接视图和数据
7. 双向数据绑定的实现
...
1. MVVM 模式是由经典的软件架构 MVC 衍生来的
  - 当 View（视图层）变化时，会自动更新到 ViewModel（视图模型）
  - 反之亦然，View 和 ViewModel 之间通过双向绑定（data-binding）建立联系
  - 与 MVC 不同的是，它没有 Controller层，而是演变为 ViewModel
  - ViewModel 通过双向数据绑定把 View 层和 Model 层连接了起来
  - View 和 Model 之间的同步工作是由 Vue完成的，我们不需要手动操作 DOM，只需要维护好数据状态
  - 面向数据编程
...
2. 不支持响应式更新的，直接替换为新数组
  - `filter, concat, slice`

3. 实现原理：用 ` Object.defineProperty ` 对数组方法进行改写
